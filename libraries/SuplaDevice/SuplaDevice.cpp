...
int SuplaDeviceClass::addChannel(int pin1, int pin2, bool hiIsLo, bool bistable, int type) {
...
int SuplaDeviceClass::addRelayButton(int relayPin, int buttonPin, int type_button, bool hiIsLo, _supla_int_t functions) {
	
	int c = addChannel(relayPin, buttonPin, hiIsLo, false, type_button);
	if ( c == -1 ) return -1;
...
bool SuplaDeviceClass::addRelayButton(int relayPin, int buttonPin, int type_button) {
	return addRelayButton(relayPin, buttonPin, type_button, false, SUPLA_BIT_RELAYFUNC_CONTROLLINGTHEGATEWAYLOCK
...
bool SuplaDeviceClass::relaySwitch(int channel_number, int relay) {
 ...
 channelSetValue(channel_number, val == HIGH ? LOW : HIGH, 0); 
 }
